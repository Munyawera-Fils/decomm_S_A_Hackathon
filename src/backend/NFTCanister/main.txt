...
// Function for creating and adding an NFT to a user
public shared (_msg) func createAndAddNftToUser(userName: Text, nftName: Text, nftDescription: Text, nftUrl: Text) : async Result.Result<(), Text> {
    ...
    switch (user) {
        ...
        case (?userRecord) {
            ...
            let request : ICRC7.SetNFTItemRequest = {
                token_id = counter; // Make sure the counter increments with each NFT
                owner = ?{
                ...
                memo = ?Blob.fromArray([0, 1]); // Add memo if necessary
                override = true;
                created_at_time = null;
            };
...
// Get NFTs by user
public shared (_msg) func getNFTsByUser(principal: Principal) : async [Nat] {
    let nfts = await ICRC7.tokens_of(principal, null, null);
    return nfts;
};
        ...
        case (?userObj) {
            ...
            switch (productResult) {
                ...
                case (#ok(product)) {
                    ...
                    // Prepare NFT transfer arguments
                    ...
                    let icrc7Result = await ICRC7.transfer([transferArgs]);
