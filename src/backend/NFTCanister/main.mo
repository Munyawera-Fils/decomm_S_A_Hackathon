...
shared(_init_msg) actor class NFTact(_args : {
...
// Define an array to hold user wallets as tuples (name, wallet)
    ...
    public shared(msg) func nftacc(name: Text) : async Principal {
        ...
        let existing_wallets = Array.filter<(Text, Principal)>(user_wallets, func(user_wallet) {
            let (n, _) = user_wallet; // Destructure the tuple
            return n == name; // Check if the name matches
        });

        // If a wallet exists, return it
        if (Array.size(existing_wallets) > 0) {
            let (_, wallet) = existing_wallets[0]; // Get the first matching wallet
            return wallet;
        };
        let fuzz = Fuzz.Fuzz();
        let p1 = fuzz.principal.randomPrincipal(10);
        let p2 = Principal.toText(p1); // => "4u4hq-3aaae-bagaz-jaeba-kaq"

        // If the wallet does not exist, create a new one with a unique principal
        let new_wallet = Principal.fromText(p2); // Generates a new unique principal
        // log the principal
        
        user_wallets := Array.append(user_wallets, [(name, new_wallet)]);
        return new_wallet;
    };

    public shared(msg) func nftaccs() : async [(Text, Principal)] {
        return user_wallets;
    };
    
    public shared(msg) func nft_create_token(metadata: ICRC7.SetNFTRequest) : async [ICRC7.SetNFTResult] {
      
        switch(icrc7().set_nfts<system>(msg.caller, metadata, true)) {
            case (#ok(val)) D.trap("okay"); // Return the result if successful
            case (#err(err)) D.trap(err); // Handle errors by trapping
        };
    };

    // assign token to user wallet (where i have to specify name)
    public shared(msg) func nft_assign_token(token_id: Nat, name: Text) : async Nat {
        let wallet = await nftacc(name);
        switch(icrc7().transfer<system>(Principal.fromActor(this), [{
            from_subaccount = null;
   ...
   public shared func getBalanceByUsername(username: Text): async Nat {
    ...
    // icrc7_tokens_of
    public shared(msg) func nft_tokens_of(name: Text) : async [Nat] {
        let wallet = await nftacc(name);
            ...
            switch (productResult) {
                ...
                case (#ok(product)) {
                    ...
                    switch (seller) {
                        ...
                        case (?seller) {
                            let buyerWallet = await nftacc(buyerName);
                            let sellerWallet = await nftacc(sellerName);
                            ...
                            let sellerTokens = await nft_tokens_of(sellerName);

                            // check if user returned array and log it 
