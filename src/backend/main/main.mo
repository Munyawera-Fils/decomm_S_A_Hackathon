import NFTact "canister:NFT_canister";
...
actor class Main() {
    ...
    // funct for public shared (_msg) func payWithNFT(buyerName: Text, productID: Nat) : async Result.Result<(), Text> {
    // nft payment
...
public shared (_msg) func payWithNFT(buyerName: Text, productID: Nat) : async Result.Result<(), Text> {
    ...
    let userObjOpt = await getUserByName(buyerName);
    ...
    switch (userObjOpt) {
        ...
        case (?userObj) {
            ...
            switch (productResult) {
                ...
                case (#ok(product)) {
                    let sellerName = await product.getSellerID();
                    let seller = await getUserByName(sellerName);
                    ...
                    switch (seller) {
                        ...
                        case (?seller) {
                            let buyerWallet = await NFTact.nftacc(buyerName);
                            let sellerWallet = await NFTact.nftacc(sellerName);

                            let buyerBalance = await NFTact.getBalanceByUsername(buyerName);
                            let sellerBalance = await NFTact.getBalanceByUsername(sellerName);
                            let productPrice = await product.getPrice();
                            // price has : {amount : Nat; currency : Currency}
                            let price = productPrice.amount;

                            // tokens of name
                            let buyerTokens = await NFTact.nft_tokens_of(buyerName);
                            let sellerTokens = await NFTact.nft_tokens_of(sellerName);
                            ...
                            for (j in Iter.range(0, (price-1))) {
                                ...
                                var transferResult = await NFTact.nft_transfer_token(token_id, buyerName, sellerName);
